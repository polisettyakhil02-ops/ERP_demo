...
router.get('/', requireRole('finance', 'consultant'), async (req, res, next) => {
  try {
    const { type, from_date, to_date, branch } = req.query
    const where = {}
...
router.post('/', requireRole('finance', 'consultant'), validate(incomeSchema), async (req, res, next) => {
  try {
    const branch = req.body.branch || req.user.branch || undefined
    const item = await prisma.income.create({ data: { ...req.body, branch, created_by: req.user.id } })
...
router.post('/bulk', requireRole('finance'), async (req, res, next) => {
  try {
    ...
    const created = await prisma.income.createMany({ data: parsed })
...
router.put('/:id', requireRole('finance'), validate(updateSchema), async (req, res, next) => {
  try {
    ...
    const item = await prisma.income.update({ where: { id: req.params.id }, data: { ...data, updated_by: req.user.id } })
    res.json(item)
  } catch (err) { next(err) }
})
